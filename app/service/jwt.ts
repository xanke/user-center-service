...
export default abstract class JwtService extends Service {
  ...
  public async verify(token: string) {
    // 获取解密成功的数据
    const result = await this.app.jwt.verify(
      token,
  ...
  public async blackToken(token: string) {
    ...
    // 获取 token 失效时间
    const refreshTtl =
      jwtData.payload.iat + this.app.config.jwt_extra.refresh_ttl * 60 * 60;
    ...
    // 储存到缓存黑名单中去
    return this.ctx.handlers.cache.set(
